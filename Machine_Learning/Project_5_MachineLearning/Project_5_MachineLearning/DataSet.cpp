...
int DataSet::getBestSplitAtt() const // NEW for P3 // MODIFIED for P5
{
...
int DataSet::getBestSplitAtt(vector<int> indices) const // NEW for P5
{
    int bestSplitAtt = 0;
    double bestGain = 0.00;
    
    for (int i = 0; i < indices.size(); i++) { // - 1 so as to ignore class label
        
        double currGain = this->gain(indices[i]);
        
        if (currGain > bestGain) {
            bestSplitAtt = indices[i];
            bestGain = currGain;
        }
    }
...
double DataSet::gain(int attribute) const  // NEW for P3
{
    ...
    for (int i = 0; i < splits.size(); i++) {
        ...
        if (examplesPer_attValue[i] != 0) {
            entropySum += ((examplesPer_attValue[i] / double(examples.size())) * splits[i].entropy());   // NEED TO MODIFY THIS FOR P5
        }
    }
...
vector<DataSet> DataSet::bootstrap(int k) const
{
    vector<DataSet> bootstraps;
    srand ( time(NULL) );
    // using folds
    
    
    for (int i = 0; i < k; i++) {                       // create bootstraps
        
        DataSet ds_bootstraped = * new DataSet(this->getAttributes());
        while (ds_bootstraped.getExamples().size() < this->getExamples().size()) {
            
            int randIndex = rand() % this->getExamples().size();
            ds_bootstraped.add(this->getExamples()[randIndex]);
            
        }
        bootstraps.push_back(ds_bootstraped);
